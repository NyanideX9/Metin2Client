...
bool CEffectData::LoadScript(const char * c_szFileName)
{
	...
	std::string strDayType;
	if (TextFileLoader.GetTokenString("daytype", &strDayType))
	{
		if (0 == strDayType.compare("DAY"))
			m_eDayType = EEffectDayType::EFFECT_DAYTYPE_DAY;
		else if (0 == strDayType.compare("NIGHT"))
			m_eDayType = EEffectDayType::EFFECT_DAYTYPE_NIGHT;
	}
#endif
...
{
#if defined(__BL_EFFECT_DAY_TYPE__)
	m_eDayType = EEffectDayType::EFFECT_DAYTYPE_ALL;
#endif
	m_fBoundingSphereRadius = 0.0f;
	m_v3BoundingSpherePosition.x = m_v3BoundingSpherePosition.y = m_v3BoundingSpherePosition.z = 0.0f;

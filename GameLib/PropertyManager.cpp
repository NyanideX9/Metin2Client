...
bool CPropertyManager::Initialize(const char * c_pszPackFileName)
{
	...
	{
		m_isFileMode = true;
		// NOTE : ż©±âĽ­ Property¸¦ µî·Ď˝ĂĹ°¸é WorldEditorżˇĽ­ ŔĚ»óŔĚ »ý±č ;
		//        ¶ÇÇŃ, Property Tree Listżˇµµ µî·ĎŔ» ˝ĂÄŃľß µÇ±â ¶§ą®żˇ ąŮ±ůÂĘżˇĽ­.. - [levites]
	}
...
bool CPropertyManager::Put(const char * c_pszFileName, const char * c_pszSourceFileName)
{
	...
	if (!m_isFileMode)	// ĆŃ ĆÄŔĎżˇµµ łÖŔ˝
	{
...
bool CPropertyManager::Erase(DWORD dwCRC)
{
	...
	if (!m_isFileMode)	// ĆÄŔĎ ¸đµĺ°ˇ ľĆ´Ď¸é ĆŃżˇĽ­µµ Áöżň
		m_pack.Delete(pProperty->GetFileName());

	FILE * fp = fopen("property/reserve", "a+");

	if (!fp)
		LogBox("żąľŕ CRC ĆÄŔĎŔ» ż­ Ľö ľř˝Ŕ´Ď´Ů.");
	else
	{
		char szCRC[64 + 1];

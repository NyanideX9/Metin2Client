...
/*/
			//@natanek5			Fix Reading d:/ymir work, FPS Drops, Slow Loading Time
bool CEterPackManager::Get(CMappedFile& rMappedFile, const char* c_szFileName, LPCVOID* pData)
{
	const bool isAbsolute = (c_szFileName[1] == ':');

	switch (m_iSearchMode)
	{
		case SEARCH_PACK_FIRST:
		{
			if (GetFromPack(rMappedFile, c_szFileName, pData))
			{
				return true;
			...
			if (!isAbsolute && GetFromFile(rMappedFile, c_szFileName, pData))
			{
#ifdef _DEBUG
...
bool CEterPackManager::isExistInPack(const char * c_szFileName)
{
	...
	// NOTE : ¸ĹÄˇ µÇ´Â ĆŃŔĚ ľř´Ů¸é false - [levites]
	return false;
#endif
}
...
void CEterPackManager::RetrieveHybridCryptPackKeys(const BYTE *pStream)
{
	...
	for( int i = 0; i < iPackageCnt; ++i )
	{
		...
		iRecvedCryptKeySize -= sizeof(dwPackageNameHash); // Ľ­ąöżˇĽ­ ąŢŔş key streamżˇ´Â filename hash°ˇ Ć÷ÇÔµÇľî ŔÖŔ¸ąÇ·Î, hash »çŔĚÁî ¸¸Ĺ­ ąčÁÜ.

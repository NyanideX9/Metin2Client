...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pMarkInstance && pGuildNameInstance)
		{
			...
			pMarkInstance->SetDepth(pTextTail->z); //@natanek165
...
void CPythonTextTail::ShowCharacterTextTail(DWORD VirtualID)
{
	...
	const auto isShowOutline = system.GetTextShadowMode();
	...
	{
		if (isShowOutline)
			pTextTail->pOwnerTextInstance->SetOutline(true);
		else
			pTextTail->pOwnerTextInstance->SetOutline(false);
	}
#endif
...
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char * c_szName)
{
	...
	if (pInstanceBase)
	{
		...
		{
			pTextTail->pOwnerTextInstance->SetColor(0.0f, 1.0f, 0.0f);
		}
	}
	//@natanek29

...
void CPythonMiniMap::__StartZoomProcess(float fTargetScale)
{
	m_fTargetBeginScale = m_fScale;
	m_fTargetEndScale = fTargetScale;
...
{
	return m_fTargetEndScale != m_fScale;
}
#endif
...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	if (__IsZoomProcess())
	{
		const DWORD dwCurMSec = CTimer::Instance().GetCurrentMillisecond();
		if (dwCurMSec <= m_dwZoomEndTime)
		{
			const float fRatio = static_cast<float>(dwCurMSec - m_dwZoomBeginTime) / g_dwMinimapSmoothZoomMSec;
			SetScale(GetLinearInterpolation(m_fTargetBeginScale, m_fTargetEndScale, __EaseOutQuad(fRatio)));
		}
		else
		{
			SetScale(m_fTargetEndScale);
		}
	}
#endif
...
{
	__StartZoomProcess(fMIN(m_fTargetEndScale * 2.0f, 4.0f));
}

void CPythonMiniMap::ScaleDown()
{
	__StartZoomProcess(fMAX(m_fTargetEndScale * 0.5f, 0.5f));
}
#else
void CPythonMiniMap::ScaleUp()
{
...
void CPythonMiniMap::__Initialize()
{
	...
	m_fScale = 2.0f;
#if defined(__BL_MINIMAP_SMOOTH_ZOOM__)
	m_fTargetBeginScale = m_fTargetEndScale = m_fScale;
	m_dwZoomBeginTime = m_dwZoomEndTime = 0;
#endif

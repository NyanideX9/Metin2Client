...
#ifdef LOTERIA_MOJA
			case HEADER_GC_FORTUNE_INFO:
				ret = RecvFortuneInfoPacket();
				break;
#endif
...
void CPythonNetworkStream::SetGamePhase()
{
	...
	CPythonCharacterManager::Instance().TouchViewBoundDistance(CPythonSystem::Instance().GetExtremeFpsBoost());
#endif
...
bool CPythonNetworkStream::SendExchangeItemAddPacket(TItemPos ItemPos, BYTE byDisplayPos, bool isRightClickAutoMoveEnabled) //@natanek135
{
	if (!__CanActMainInstance())
		return true;
	...
	packet.isRightClickAutoMoveEnabled = isRightClickAutoMoveEnabled; //@natanek135
...
bool CPythonNetworkStream::RecvWeeklyRank()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SendWeeklyInfo", Py_BuildValue("(isi)", rPlayerWeekly.rank, rPlayerWeekly.name, rPlayerWeekly.points));
	}
	return true;
}
#endif
...
bool CPythonNetworkStream::RecvItemShopPacket()
{
	...
	{
		WORD wCount = (kItemShop.size - sizeof(TItemShopGCPacket)) / sizeof(TItemShopItem);
	...
	case ITEMSHOP_SUBHEADER_GC_COINS:
	{
		...
		TItemShopCashPacket pack{};
		if (!Recv(sizeof(TItemShopCashPacket), &pack))
			return false;

		CItemShop::Instance().SetCoins(pack.value, pack.type);
	...
	case ITEMSHOP_SUBHEADER_GC_OPEN:
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ItemShopOpen", Py_BuildValue("(ii)", pack.byDailyRewardDay, pack.ttDataTime));
#else
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ItemShopOpen", Py_BuildValue("(iL)", pack.byDailyRewardDay, pack.ttDataTime));
#endif
		break;
	}
	...
	case ITEMSHOP_SUBHEADER_GC_REWARD:
	{
		BYTE byItemCount = 0;
		BYTE byDay = 0;

		if (!Recv(sizeof(BYTE), &byDay) || !Recv(sizeof(BYTE), &byItemCount))
			return false;

		for (auto i = 0; i < byItemCount; i++)
		{
			TItemShopDailyReward reward;
			if (!Recv(sizeof(TItemShopDailyReward), &reward))
				return false;

			CItemShop::Instance().SetDailyReward(byDay, reward);
		}

		break;
	}
	case ITEMSHOP_SUBHEADER_GC_REWARD_STATUS:
	{
		BYTE byGotReward = 0;

		if (!Recv(sizeof(BYTE), &byGotReward))
			return false;

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ItemShopSetRewardStatus", Py_BuildValue("(i)", byGotReward));
...
bool CPythonNetworkStream::SendItemShopBuyPacket(DWORD id, DWORD category, BYTE count, BYTE bonus_type)
{
	if (!__CanActMainInstance())
		return true;
...
bool CPythonNetworkStream::SendItemShopGetDailyRewardPacket()
{
	if (!__CanActMainInstance())
		return true;
...
#ifdef LOTERIA_MOJA
bool CPythonNetworkStream::RecvFortuneInfoPacket()
{
	TPacketFortuneItemsInfo pFortuneInfoPacket{};

	if (!Recv(sizeof(TPacketFortuneItemsInfo), &pFortuneInfoPacket))
	{
		Tracen("Recv Fortune Info Packet Error");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_PrepareFortune", 
	Py_BuildValue("(i)", pFortuneInfoPacket.RewardIndex));

	for (int i = 0; i < 80; i++)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_FortuneAddItem", 
		Py_BuildValue("(i)", pFortuneInfoPacket.dwItemVnums[i]));
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RunFortune", Py_BuildValue("()"));

	return true;
}
#endif

#ifdef ENABLE_ENTITY_PRELOADING		//kodo kod

...
//	rkNetStream.SendExchangeItemAddPacket(TItemPos(bWindowType, wSlotIndex), iDisplaySlotIndex);
	rkNetStream.SendExchangeItemAddPacket(TItemPos(bWindowType, wSlotIndex), iDisplaySlotIndex, iDisplaySlotIndex == -1); //@natanek135
	return Py_BuildNone();
}
...
PyObject* netSendSafeboxCheckinPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	int iSafeBoxPos;
	bool isRightClickAutoMoveEnabled = false; //@natanek135
...
//	rns.SendSafeBoxCheckinPacket(InventoryPos, iSafeBoxPos);
	rns.SendSafeBoxCheckinPacket(InventoryPos, iSafeBoxPos, isRightClickAutoMoveEnabled); //@natanek135
...
//	rns.SendSafeBoxCheckoutPacket(iSafeBoxPos, InventoryPos);
	rns.SendSafeBoxCheckoutPacket(iSafeBoxPos, InventoryPos, isRightClickAutoMoveEnabled); //@natanek135
...
//	rns.SendMallCheckoutPacket(iMallPos, InventoryPos);
	rns.SendMallCheckoutPacket(iMallPos, InventoryPos, isRightClickAutoMoveEnabled); //@natanek135
...
PyObject* netSendItemShopGetDailyRewardPacket(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendItemShopGetDailyRewardPacket();
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
        ...
        { "SendItemShopGetDailyRewardPacket", netSendItemShopGetDailyRewardPacket, METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "ACCOUNT_CHARACTER_SLOT_LAST_PLAYTIME", CPythonNetworkStream::ACCOUNT_CHARACTER_SLOT_LAST_PLAYTIME); //@natanek25

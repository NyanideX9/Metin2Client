...
#ifdef ENABLE_NPC_TARGET_EFFECT_FIX
	for (std::map<uint32_t, std::set<uint32_t>>::const_iterator it = m_map_NPCTargetEffect.begin(); it != m_map_NPCTargetEffect.end(); ++it)
	{
		...
		CreateSpecialEffect(vid, +kPixelPosition.x, -kPixelPosition.y, +kPixelPosition.z, g_strEffectName.c_str());
	}
#endif
...
#ifdef ENABLE_NPC_TARGET_EFFECT_FIX
			if (m_map_NPCTargetEffect.find(dwChrID) != m_map_NPCTargetEffect.end())
				continue;
#endif
...
#ifdef ENABLE_NPC_TARGET_EFFECT_FIX
	CInstanceBase* pInst = CPythonCharacterManager::Instance().GetInstancePtr(dwChrVID);
	if (pInst && pInst->IsNPC())
	{
		if (m_map_NPCTargetEffect.find(dwChrVID) == m_map_NPCTargetEffect.end())
			m_map_NPCTargetEffect.insert(std::make_pair(dwChrVID, std::set<uint32_t>{dwID}));
		else
			m_map_NPCTargetEffect[dwChrVID].insert(dwID);
	}
#endif
}
...
#ifdef ENABLE_NPC_TARGET_EFFECT_FIX
		std::map<DWORD, DWORD>::iterator itChrID = m_kMap_dwTargetID_dwChrID.find(dwID);
		if (itChrID != m_kMap_dwTargetID_dwChrID.end())
		{
			DWORD chrID = itChrID->second;
			std::map<uint32_t, std::set<uint32_t> >::iterator itNPC = m_map_NPCTargetEffect.find(chrID);
			if (itNPC != m_map_NPCTargetEffect.end())
			{
				itNPC->second.erase(dwID);
				if (itNPC->second.empty())
				{
					DeleteSpecialEffect(chrID);
					m_map_NPCTargetEffect.erase(itNPC);
				}
				m_kMap_dwTargetID_dwChrID.erase(itChrID);
				return;
			}
		}
#endif

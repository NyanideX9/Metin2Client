...
//@natanek6 - Shaman buff target visual bug
...
//@natanek6 - Shaman buff target visual bug
				...
				!pkInstTarget->IsStoneDoor())))
		{
			...
			{
				Tracenf("CPythonPlayer::__UseSkill(%d) - ¥È¡©?¥é ¡¾¥â¥Á¥× ?¥æ¥Ç¥â ?©ø¥Á¢´?¡í ¥Ç¥×??¥Ç¥Ó", dwSlotIndex);
			}
		}
//@natanek6 - Shaman buff target visual bug
...
/*/
	...
	if (dwTargetMaxCount>0 && pkInstTarget)
	{
		...
		if (pSkillData->IsFanRange())
		{
			...
			//fix arrow shower x5 damage @natanek150
...
#define COMBO_FIX
		...
		if (pkInstMain->IsAttacking()){
			CPythonChat::Instance().AppendChat(CHAT_TYPE_INFO, "Combo mo¿na w³¹czyæ/wy³¹czyæ kiedy nie atakujesz.");
			return;
		}
#endif

...
#ifdef LOTERIA_MOJA
	HEADER_GC_FORTUNE_INFO = 214,
#endif

#ifdef ENABLE_IKASHOP_RENEWAL
	HEADER_GC_NEW_OFFLINESHOP					= 215,
#endif
...
typedef struct command_exchange
{
	...
	BYTE		arg2;
	TItemPos	Pos;
	bool isRightClickAutoMoveEnabled; //@natanek135
} TPacketCGExchange;
...
typedef struct SSimplePlayerInformation
{
	...
	WORD				wPort;
	BYTE				bySkillGroup;
	uint32_t dwLastPlayTime; //@natanek25
...
#ifdef LOTERIA_MOJA
typedef struct SPacketFortuneItemsInfo
{
	BYTE	header;
	DWORD	dwItemVnums[FORTUNE_REWARD_COUNT];
	BYTE	RewardIndex;
} TPacketFortuneItemsInfo;
#endif

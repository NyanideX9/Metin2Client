...
void ResultItemFromData(TItemShopItem item, PyObject* pItem)
{
	...
	PyDict_SetItemString(pItem, "price_type", Py_BuildValue("i", item.price_type));
#endif
...
void CItemShop::SetDailyReward(BYTE byDay, TItemShopDailyReward reward)
{
	PyCallClassMemberFunc(m_pyItemShopWnd, "BINARY_SetDailyReward", Py_BuildValue("(iii)", byDay, reward.dwVnum, reward.byCount));
}
...
#ifdef ENABLE_EXTENDED_COIN
CItemShop::CItemShop() : m_dwCoins{}, m_pyItemShopWnd{ nullptr }
{
	memset(m_dwCoins, 0, sizeof(m_dwCoins));
...
PyObject* itemshopGetCoins(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("i", CItemShop::Instance().GetCoins(type));
#else
...
#ifdef ENABLE_EXTENDED_COIN
	PyModule_AddIntConstant(poModule, "ENABLE_EXTENDED_COIN", 1);
	PyModule_AddIntConstant(poModule, "CASH_TYPE_COIN", CASH_TYPE_COIN);
	PyModule_AddIntConstant(poModule, "CASH_TYPE_MARK", CASH_TYPE_MARK);
	PyModule_AddIntConstant(poModule, "CASH_TYPE_MAX_NUM", CASH_TYPE_MAX_NUM);
#else
...
#ifdef ENABLE_EXTENDED_COIN_SPLIT_CATEGORY
	PyModule_AddIntConstant(poModule, "ENABLE_EXTENDED_COIN_SPLIT_CATEGORY", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EXTENDED_COIN_SPLIT_CATEGORY", 0);
#endif

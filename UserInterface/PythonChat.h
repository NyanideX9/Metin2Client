...
//		typedef std::map<std::string, CWhisper*> TWhisperMap;
		using TWhisperMap = std::map<std::string, std::unique_ptr<CWhisper>>; //@natanek78
	...
	public:
		...
		bool GetChatMode(uint32_t chatSetId, int32_t* chatModeFlags); //natanek61
...
//		CWhisper * CreateWhisper(const char * c_szName);
		CWhisper* CreateWhisper(const char* targetName); //@natanek78

...
//		if( rkActorEach->IsPC() || rkActorEach->IsNPC() || rkActorEach->IsEnemy() )
		if( rkActorEach->IsPC() || rkActorEach->IsNPC() || rkActorEach->IsEnemy() || rkActorEach->IsStone()) // @natanek4 //IsStone() fix teleport lag by MariusExygo
...
bool CNetworkActorManager::__IsVisiblePos(LONG lPosX, LONG lPosY)
{
	...
	const int viewDist = CPythonCharacterManager::instance().GetViewBoundDistance();
	if (len < viewDist)
		return true;
#else

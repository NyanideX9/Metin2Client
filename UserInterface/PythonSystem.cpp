...
void CPythonSystem::SetExtremeFpsBoost(bool bVal)
{
	m_Config.bExtremeFpsBoost = bVal;
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	rkChrMgr.TouchViewBoundDistance(bVal);
}
#endif
...
void CPythonSystem::SetDefaultConfig()
{
	...
	m_Config.bGreenTexturesMode = false;
#endif
	...
	m_Config.bCameraMode = 0; //@natanek97
...
bool CPythonSystem::IsGreenTexturesMode()
{
	return m_Config.bGreenTexturesMode;
}

void CPythonSystem::SetGreenTexturesMode(int iFlag)
{
	m_Config.bGreenTexturesMode = iFlag == 1 ? true : false;
}
#endif
...
bool CPythonSystem::GetTextShadowMode()
{
	return m_Config.bTextShadowMode;
}

void CPythonSystem::SetTextShadowMode(int iFlag)
{
	m_Config.bTextShadowMode = iFlag == 1 ? true : false;
}
#endif
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp (command, "FOG_MODE_ON"))
		{
			...
			m_Config.bSnowTexturesMode = atoi(value);
		else if (!stricmp(command, "DESERT_TEXTURES_MODE"))
			m_Config.bDesertTexturesMode = atoi(value);
		else if (!stricmp(command, "GREEN_TEXTURES_MODE"))
			m_Config.bGreenTexturesMode = atoi(value);
#endif
...
#ifdef EXTREME_FPS_BOOST
		else if (!stricmp(command, "EXTREME_FPS_BOOST")) 
			m_Config.bExtremeFpsBoost = atoi(value) == 1 ? true : false; 
#endif
...
#ifdef ENABLE_TEXT_SHADOW
		else if (!stricmp(command, "TEXT_SHADOW_MODE"))
			m_Config.bTextShadowMode = atoi(value) == 1 ? true : false;
#endif
...
bool CPythonSystem::SaveConfig()
{
	...
	if (m_Config.bShowSalesText == 0)
		fprintf(fp, "SHOW_SALESTEXT		%d\n", m_Config.bShowSalesText);
			fprintf(fp, "CAMERA_MODE\t\t\t%d\n", m_Config.bCameraMode); //@natanek97
...
#ifdef EXTREME_FPS_BOOST
	fprintf(fp, "EXTREME_FPS_BOOST	%d\n", m_Config.bExtremeFpsBoost);
#endif

#ifdef ENABLE_TEXT_SHADOW
	fprintf(fp, "TEXT_SHADOW_MODE		%d\n", m_Config.bTextShadowMode);
#endif

#ifdef ENABLE_TEXTURE_CHANGER
	fprintf(fp, "SNOW_TEXTURES_MODE	%d\n", m_Config.bSnowTexturesMode);
	fprintf(fp, "DESERT_TEXTURES_MODE %d\n", m_Config.bDesertTexturesMode);
	fprintf(fp, "GREEN_TEXTURES_MODE %d\n", m_Config.bGreenTexturesMode);
#endif

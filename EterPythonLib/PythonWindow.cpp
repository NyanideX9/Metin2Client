		...
		if (IsFlag(FLAG_ANIMATED_BOARD))
		{
			if (!this->m_bShow)
			{
				...
				m_sizeAnimation = tweeny::from(0.0f)
					.to(1.0f)
					.during(175)
// Zmieniając wartości powyżej, możemy dostosować szybkość animacji.
// Natomiast cubicOut poniżej to efekt, możemy go zmieniać na inny wymieniony w enum class enumerated w pliku tweeny-3.2.0.h
...
BOOL CBar3D::OnIsType(DWORD dwType)
{
	...
	void CTextLine::SetFontName(const char * c_szFontName)
	{
		...
		m_TextInstance.DisableUpdate();
		m_TextInstance.Update();
	#endif
	}
	#ifdef ENABLE_DYNAMIC_FONTS
	void CTextLine::SetFontDynamic(bool f)
	{
		m_TextInstance.SetDynamicFont(f);
	}
	#endif
...
#ifdef ENABLE_OFFICIAL_REVERSED_BUTTON
	...
	CButton::CButton(PyObject * ppyObject)
		:	CWindow(ppyObject),
			...
			m_bAlwaysExecuteOverFunc(false)
#else
			m_isFlash(FALSE)
#endif
	...
	void CButton::SetAlwaysExecuteOverFunc(bool bVal)
	{
		m_bAlwaysExecuteOverFunc = bVal;
	}
		...
		if (m_pcurVisual)
		{
			...
			if (int(timeGetTime() / 500)%2)
			{
				if (m_pFlashVisual)
					...
					m_pFlashVisual->Render(m_bEnableMask ? &m_rMaskRect : NULL);
#else
	...
	void CButton::OnMouseOverIn()
	{
		...
		if (m_bAlwaysTooltip)
		{
			if (bEnable)
				Over();
			else if (m_bAlwaysExecuteOverFunc)
				PyCallClassMemberFunc(m_poHandler, "OnMouseOverIn", BuildEmptyTuple());

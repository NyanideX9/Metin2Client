...
{
	return m_pgrnFileInfo ? max(0, m_pgrnFileInfo->TextureCount) : 0;
}

std::string_view CGraphicThing::GetTexturePath(int32_t textureIndex) const
{
	if (!m_pgrnFileInfo || textureIndex < 0 || textureIndex >= m_pgrnFileInfo->TextureCount) 
	{
		return "";
	}

	return std::string_view(m_pgrnFileInfo->Textures[textureIndex]->FromFileName);
}
	//@natanek65

...
static CGraphicText* pkDefaultFont_RELOAD = NULL;

typedef std::list < CGraphicTextInstance * > t_GraphicTextInstances;
t_GraphicTextInstances GraphicTextInstances;

extern void GraphicTextInstancesReload(CGraphicText* pkDefaultFont)
{
	pkDefaultFont_RELOAD = pkDefaultFont;

	t_GraphicTextInstances::iterator it = GraphicTextInstances.begin();
	for (; it != GraphicTextInstances.end(); it++)
	{

		CGraphicTextInstance * t = *(it);
		if (!t || t->GetDynamicFontOff())
		{
			continue;
		}

		t->SetTextPointer(pkDefaultFont_RELOAD);
		t->DisableUpdate();
		t->Update();
	}
}

void __GraphicTextInstancesList(CGraphicTextInstance * t, bool add = true)
{
	if (!t)
	{
		return;
	...
	if (add)
	{
		GraphicTextInstances.push_back(t);
		return;
	}

	t_GraphicTextInstances::iterator it = std::find(GraphicTextInstances.begin(), GraphicTextInstances.end(), t);
	if (it != GraphicTextInstances.end())
	{
		GraphicTextInstances.erase(it);
	}
}
#endif
...
//	std::map<CGraphicImageTexture*, std::vector<SPDTVertexRaw>> verticesMap;
	std::unordered_map<CGraphicImageTexture*, std::vector<SPDTVertexRaw>> verticesMap; //Poprawka GPT
	...
	for (auto& p : verticesMap)
	{
		...
		{
			 auto batchCount = std::min<std::size_t>(LARGE_PDT_VERTEX_BUFFER_SIZE,
				l - f);
...
{
#ifdef ENABLE_DYNAMIC_FONTS
	CGraphicTextInstance* t = ms_kPool.Alloc();
	__GraphicTextInstancesList(t);
	return t;
#else
...
{
#ifdef ENABLE_DYNAMIC_FONTS
	__GraphicTextInstancesList(pkInst, false);
#endif
...
{
#ifdef ENABLE_DYNAMIC_FONTS
	if (pkDefaultFont_RELOAD && !dynamic_font_off)
	{
		m_roText = pkDefaultFont_RELOAD;
		return;
	}
#endif
...
#ifdef ENABLE_DYNAMIC_FONTS
	__GraphicTextInstancesList(this, false);
#endif
...
#ifdef ENABLE_DYNAMIC_FONTS
	__GraphicTextInstancesList(this);
#endif
}
